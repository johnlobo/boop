...
Amstrad CPC game **boop**, Z80 assembly, **CPCtelera** engine. Mode 0 (160x200, 16 colors). Memory layout: binary loads at `0x0100` (transparency table), code at `0x0200` (`Z80CODELOC` in `cfg/build_config.mk`), entry point (`_main`) linked at approximately `0x2BDF`.
...
- **game** (`sys_game_init` / `sys_game_update`) - Top-level state machine with four states: `GAME_STATE_MENU=0`, `GAME_STATE_PLAYING=1`, `GAME_STATE_HELP=2`, `GAME_STATE_AI_SELECT=3`. Menu confirmed=1 (ONE PLAYER) → AI_SELECT (difficulty picker); confirmed=2 (TWO PLAYERS) → PLAYING directly; confirmed=3 → HELP. ESC on AI-select returns to menu. Also owns music-track switching on every state transition.
...
**Simulation** — `_ai_score_one_candidate` saves board + both player structs (36+6+6B) and cursor state, enables `_match_simulation_mode`, calls `_ai_place_no_animate` (tail-calls `_match_boop`/`_match_boop_cat`), resolves mixed trios in real scan order via `_ai_resolve_p2_lines`, checks cat-line wins through the shared table scanner `_ai_has_cat_win`, applies empty-reserve graduation/win, evaluates, then restores all saved state. Simulation mode suppresses eject SFX and HUD cat-reserve blink markers; animation and winner dialogs are also omitted.
