...
_smlwd_height_ok:
    ...
    ;; Bounded string scan: require a terminator within 37 characters.
    ;; This prevents both width arithmetic overflow and text drawing outside
    ;; the window; sys_text_str_length itself has no maximum-length guard.
    ld h, w_message+1(iy)
    ld l, w_message(iy)
...
_smlwd_width_min_ok:
    ...
    ;; Captured background size is exactly width*height bytes. Verify it
    ;; against the fixed 3000-byte buffer with the existing 8x8 multiplier.
    ld e, w_w(iy)
    ld h, w_h(iy)
